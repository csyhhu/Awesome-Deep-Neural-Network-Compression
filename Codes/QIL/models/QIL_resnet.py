...
from utils.QIL import QIL_CNN, QIL_QuantAct


def conv3x3(in_planes, out_planes, stride=1, bitW=1):
    " 3x3 convolution with padding "
    return QIL_CNN(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False, bitW=bitW)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 layer_idx=None, block_idx=None, layer_name_list = None, bitW=2, bitA=2):
        super(BasicBlock, self).__init__()

        self.bitW = bitW
        self.bitA = bitA

        self.conv1 = conv3x3(inplanes, planes, stride, bitW=self.bitW)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes, bitW=self.bitW)
        self.bn2 = nn.BatchNorm2d(planes)
        self.downsample = downsample
        self.stride = stride

        self.conv1_in = QIL_QuantAct(bitA = self.bitA)
        self.conv2_in = QIL_QuantAct(bitA = self.bitA)
        if self.downsample is not None:
            self.downsample_in = QIL_QuantAct(bitA = self.bitA)
    ...
    def forward(self, x):
        ...
        out = self.conv2_in(out)
        out = self.conv2(out)
        out = self.bn2(out)

        if self.downsample is not None:
            residual = self.downsample(self.downsample_in(x))
...
class ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, first_stride=1, bitW=2, bitA=2):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, layer_idx=0):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                # QIL_QuantAct(bitA = self.bitA),
                QIL_CNN(self.inplanes, planes * block.expansion,
        ...
        layers.append(block(self.inplanes, planes, stride, downsample,
                            layer_idx=layer_idx, block_idx=0, layer_name_list=self.layer_name_list,
