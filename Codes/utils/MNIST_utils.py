...
class MNIST(data.Dataset):
    ...
    def __init__(self, root, train=True, add_split=None, transform=None, target_transform=None, download=False, ratio=-1):
        ...
        if self.train:
            ...
            if ratio != -1:
                assert (add_split == None)
                sample_length = int(ratio * len(self.train_data))
                # random_idx = np.random.randint(len(self.train_data), size=sample_length)
                random_idx = np.random.choice(len(self.train_data), size=sample_length, replace=False)
            ...
            if add_split is not None:
                assert (ratio == -1)
                if os.path.exists(os.path.join(self.root, self.processed_folder, '%s.pt' % add_split)):
                    self.train_data, self.train_labels = torch.load(
                        os.path.join(self.root, self.processed_folder, '%s.pt' % add_split))
                else:
                    print('%s file not exist, create.' %(add_split))
                    # 10% of labeled data
                    label_length = int(0.01 * len(self.train_data))
                    ...
                    # Separate label data
                    label_data = self.train_data[label_index, :]
                    label_labels = itemgetter(*label_index)(self.train_labels)
                    with open(os.path.join(self.root, self.processed_folder, 'label.pt'), 'wb') as f:
                    ...
                    unlabel_data = self.train_data[unlabel_index, :]
                    unlabel_labels = itemgetter(*unlabel_index)(self.train_labels)
                    with open(os.path.join(self.root, self.processed_folder, 'unlabel.pt'), 'wb') as f:
